...
> members through `POST /admin/local-members`; credentials are generated by the
> server and returned once after an atomic commit. There is no public sign-up
> endpoint. Broader member/tenant/policy lifecycle management, proof/evidence,
...
src/
  ...
  administration/      openproof.administration     bootstrap and owner-authorized enrollment
apps/opp/              single binary; `opp server` will run the daemon
tests/                 304 discovered tests
docs/
